...
const kafkaData = async () => {
  ...
  await admin.createTopics({
    ...
    topics: [{ topic: "stock", numPartitions: 1, replicationFactor: 1 }],
  });
    ...
    consumerVisualize.run({
      eachMessage: async ({ topic, partition, message, heartbeat, pause }) => {
        try {
          ...
          const { timestamp, price, symbol } = value;
          console.log({ viz: value })
          const time = timestamp * 1000
          io.emit("newPrice", { time, price, symbol });
        } catch (err) {
          console.log(err)
    ...
    consumerDb.run({
      eachMessage: async ({ topic, partition, message, heartbeat, pause }) => {
        try {
          ...
          const { timestamp, price, symbol } = newPrice;
          console.log({ db: newPrice });
          const time = timestamp * 1000
          await client.query(
...
app.use("/:symbol", async (req, res) => {
  try {
    ...
    if (ohlc && JSON.parse(ohlc)) {
      ...
      const queryOhlc = `select bucket AS time, open, high, low, close from ohlc where symbol = '${req.params.symbol.toUpperCase()}' and bucket >= '${new Date(
        lowerTime
      ).toISOString()}' order by bucket desc limit ${MAX}`;
      console.log(queryOhlc);
      const { rows } = await client.query(queryOhlc);
      console.log(rows);
      const lastTime = rows.at(-1)?.time
        ? new Date(rows.at(-1).time).toISOString()
        : timeNow.toISOString();
      console.log(lastTime);
      const lastDataQuery = await client.query(
        `select bucket AS time, open, high, low, close from ohlc where symbol = '${req.params.symbol.toUpperCase()}' and bucket < '${lastTime}' order by bucket desc limit 1`
      );
      ...
      while (lowerTime <= upperTime) {
        ...
        if (
          !rows.at(idx) ||
          new Date(rows.at(idx).time).valueOf() > lowerTime
        ) {
          console.log(lastQuery);
          const { close } = lastQuery;
          const newData = {
            time: new Date(lowerTime),
            open: close,
            high: close,
            low: close,
            close: close,
